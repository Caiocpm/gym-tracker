...
import type { Group, WorkoutPost, PostLike, GroupChallenge } from "../../types/social";
...
export function GroupFeed({ group, onBack }: GroupFeedProps) {
  ...
  // ✅ Estado para visualizar perfil de usuário
  const [selectedUserId, setSelectedUserId] = useState<string | null>(null);

  const isGroupAdmin = userProfile?.uid === group.createdBy;

  // ✅ Carregar posts e desafios
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("🎯 Desafios carregados:", groupChallenges.length, groupChallenges);
  ...
  // ✅ Função para abrir modal de likes
  ...
  // ✅ Função para comentários adicionados
  const handleCommentAdded = () => {
    console.log("✅ Comentário adicionado, recarregando posts");
    setRefreshKey((prev) => prev + 1);
  };
          ...
          posts.map((post) => (
            <div key={post.id} className={styles.postCard}>
              ...
              <div className={styles.postHeader}>
                ...
                >
                  {/* ✅ NOVO: Avatar com foto ou inicial */}
                ...
                {userProfile?.uid === post.userId && (
                  <button
                    ...
                    onClick={() => handleDeleteClick(post.id, post.workoutName)}
              ...
              <div className={styles.postContent}>
                ...
                <div className={styles.workoutStats}>
                  ...
                  {/* ✅ NOVO: Card de Recordes */}
                  {post.records && post.records.length > 0 && (
                    <div className={`${styles.statCard} ${styles.recordCard}`}>
                      <span className={styles.statIcon}>🏆</span>
                      <div className={styles.statContent}>
                ...
                >
                  {post.exercises.map((exercise, idx) => {
                    ...
                    // ✅ Verificar se este exercício tem recorde
                    const hasRecord = post.records && post.records.some(
                      (r) => r.exerciseName === exercise.name
                    );

                    return (
                      <div key={idx} className={`${styles.exerciseCard} ${hasRecord ? styles.recordExercise : ''}`}>
                        <div className={styles.exerciseHeader}>
                          <h4>
                            {hasRecord && <span className={styles.recordBadge}>🏆 </span>}
                            {exercise.name}
                          </h4>
              ...
              <div className={styles.postActions}>
                ...
                {/* Botão de listar likes (apenas se houver curtidas) */}
                {post.likesCount > 0 && (
                  <button
                    className={styles.actionButton}
